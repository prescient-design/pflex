...
class Orchestrator:
    ...
    def orchestrate_inf_runner_for_benchmark_set(self):
        ...
        for i in range(0, len(self.pdbs_to_fold)):
            ...
            if len(peptide) > 10 and len(peptide) < 14:
                ...
                mhc_chains = [fasta.mhc_chains[template[0].split("_")[0]]]
                pep_chains = [fasta.pep_chains[template[0].split("_")[0]]]
                template_files = [os.path.join(self.orchestrate_cfg.dir, template[0]+"_reordered.pdb")]
            ...
            if len(template) > 0:                    
                ...
                for t in range(0, len(template_files)):
                    ...
                    if pdbid not in template_files[t]:
                        ...
                        if not (starting_model == ''):
                            ...
                            compare_docked_with_native(self.homology_model_cfg.outdir, self.orchestrate_cfg.dir, filetag, pdbid)
                            compare_MD_and_docked_ensembles(self.orchestrate_cfg, len(peptide))
    ...
    def orchestrate_inf_runner_for_unk_sequences(self):
        ...
        for i in range(0, len(self.pdbs_to_fold)):
            ...
            if len(peptide) > 10 and len(peptide) < 14:
                ...
                template_for_each_pdb = closest_template_for_each_pdb_based_on_sequence(self.orchestrate_cfg.dir, fasta.pep_chains, fasta.pep_seq, self.pdbs_to_fold, False)
                print (template_for_each_pdb)
            ...
            if len(template) > 0:
                ...
                for t in range(0, len(template_files)):
                    ...
                    self.homology_model_cfg.update({"mhc_chain": mhc_chains[t]})
                    self.homology_model_cfg.update({"peptide_chain": pep_chains[t]})
                    self.homology_model_cfg.update({"outdir": os.path.join(self.orchestrate_cfg.output.outdir, pid, template[t].replace(".pdb", ""))})
                    self.homology_model_cfg.update({"allele": allele})
                    self.homology_model_cfg.update({"peptide": peptide})
